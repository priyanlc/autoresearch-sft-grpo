...
- **notes:**
  ...
  - **Why the public 0.85+ notebooks don't help (2026-05-30 scan of `notebooks/00-starters/`).** Every public starter notebook that touches vLLM gets it the same way: by sourcing the Kaggle competition's bundled `nvidia_metric_utility_script` tar (a Kaggle Utility Script blob owned by `metric/`), which uninstalls the base image's `torch / torchvision / torchaudio` and re-installs a self-contained Blackwell-targeted stack. Concrete evidence from `nvidia-nemotron-metric.ipynb`: the uninstall step's captured output shows `torch==2.9.0+cu126 / torchaudio==2.9.0+cu126 / torchvision==0.24.0+cu126`, and the bundled `ptxas` is chmod'd alongside `ptxas-blackwell` (sm_120, RTX 6000 Pro). The `LLM(...)` call uses `trust_remote_code=True` on a `model_type=nemotron_h` model — which only works because the bundled vLLM is in the 0.9.x / 1.x line where `NemotronHForCausalLM` exists natively. The one notebook that builds its own training image (`end-to-end-finetuning-for-lb-0-85.ipynb` via Modal) explicitly pins `cuda:12.8.1-devel` + `torch==2.10.0+cu128`. **No notebook in the public set runs vLLM on cu121 / torch 2.5.1 / `mamba_ssm 2.3.1` / `causal_conv1d 1.6.2.post1`.** There is no backport recipe to copy — the public stack is fundamentally a different cu126/cu128 stack. Two infra options if vLLM dev-loop eval ever becomes load-bearing: (a) **split-pod** — keep training on cu121, spin up a one-shot cu126 pod for eval cross-checks at submit time (`vllm_eval.py` is the harness, runs unchanged); or (b) **full migration** — rebuild the training pod on torch 2.10 + cu128 per the `end-to-end-finetuning` Modal recipe, re-derive `mamba_ssm` / `causal_conv1d` pins for cu128, port T2.x changes back. Option (b) is high-risk (re-opens F-013-scale drift); option (a) is cheap and one-shot. **Status quo recommendation: stay on cu121 + HF eval; defer the LB-cross-check to a one-off cu126 pod at submit time.**
