...
| T1.32 | Document `--remote-control` + `IS_SANDBOX=1` + `tmux` as the recommended throwaway-pod autonomous-launch pattern. New § "Remote Control (drive from claude.ai web/mobile)" in `docs/autoresearch-handoff.md` covering requirements (claude.ai subscription, not API keys), launch wrapper (tmux + F-010 unset + IS_SANDBOX + `--rc`), and "when NOT to use" guidance. README § "Handover" steps 1 and 3 surface the pattern. | Landed 2026-05-30 |
